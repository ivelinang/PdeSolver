...
def C(alpha, beta, rho, nu, ym, Fm):
    return np.sqrt(alpha*alpha +2.0*rho*nu*ym + nu*nu*ym*ym)*Fm**(beta)
...
def priceOptionArbFreeSabr(f, strike, T, alpha, beta, nu, rho, spot_intervals, time_intervals):
    ...
    vCall2 = [(f_min+(x-0.5)*dX-strike)*dX*myGrid[x, -1] for x in range(k+1,numXPoints-1)]
